...
%   super           struct with replicated data
    ...
    if ~isfield(unit, 'lattice') || isempty(unit.lattice)
        error('geom:build_supercell:MissingLattice', 'unit.lattice is required.');
    end

    if ~isfield(unit, 'frac_coords') || isempty(unit.frac_coords)
        error('geom:build_supercell:MissingFracCoords', 'unit.frac_coords is required.');
    end

    if ~isfield(unit, 'cart_coords') || isempty(unit.cart_coords)
        error('geom:build_supercell:MissingCartCoords', 'unit.cart_coords is required.');
    end

    if ~isnumeric(supercell_size) || numel(supercell_size) ~= 3 || any(supercell_size < 1) || any(mod(supercell_size,1) ~= 0)
    ...
    % Supercell lattice: rows are lattice vectors
    superLattice = [
        nx * lattice(1, :);
        ny * lattice(2, :);
        nz * lattice(3, :)
    ...
    hasClass = isfield(unit, 'site_class') && ~isempty(unit.site_class);
    ...
    for icell = 1:nCells
        ...
        if hasClass
            superClass(rows) = unit.site_class(:);
        end
