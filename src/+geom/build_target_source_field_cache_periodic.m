...
% Purpose
% -------
% Build an asymmetric periodic real-space target-source cache for the fixed
% charge external field:
%
%   target sites <- source charges
%
% This is used by thole.induced_field_from_charges_periodic.
%
...
%   cart = frac * H
%
% geom.get_lattice returns a canonical lattice struct with:
%
%   lat.H = H
%   lat.G such that H * G = 2*pi*I
%
% Minimum-image wrapping in this routine is done directly in the row-lattice
% convention:
%
%   frac   = dr_cart / H
%   dr_min = (frac - round(frac)) * H
%
% Inputs
%   sys              canonical polarization-system struct in atomic units:
%                      .site_pos      N x 3 Cartesian positions, bohr
%                      .super_lattice or .lattice, row-lattice H
%                      .site_alpha    needed only if opts.use_thole = true
%                      .thole_a       needed only if opts.use_thole = true
%
...
%     .target_alpha
%     .source_alpha
%     .row_ptr             (nTargets+1) x 1 CSR pointer, 1-based
%     .col_idx             nEntries x 1 source-local column indices, 1-based
%     .dr                  nEntries x 3 displacement source image -> target
%     .r_bare              nEntries x 1
%     .r2_bare             nEntries x 1
%     .inv_r3_eff          nEntries x 1 effective field coefficient:
%                            E_i = sum_j q_j * inv_r3_eff .* dr
%     .B_ewald             nEntries x 1 Ewald real-space coefficient
%     .thole_delta         nEntries x 1 local Thole correction
%     .nEntries
%     .alpha
%     .rcut
%     .Lmin
...
% geom.query_pairs_within_cutoff because that routine is designed around
% unordered source-source/subset pair queries, not arbitrary target-source
% rows.
%
% The cache assumes the same single-image regime as the periodic active
% row cache:
%
%   rcut < Lmin/2
%
...
%
% Periodic real-space Ewald charge-field contribution:
%
...
%
% If opts.use_thole = true, a local short-range Thole correction is added:
%
...
if use_thole
    ...
    target_alpha = double(alpha_all(target_sites));
    source_alpha = double(alpha_all(source_sites));
    thole_a = sys.thole_a;
else
...
if profile
    ...
    fprintf('  convention        = project row-lattice, cart = frac * H\n');
end
...
if nTargets == 0 || nSources == 0
    cache = local_empty_cache(nTargets, nSources, target_sites, source_sites, ...
        ...
        alphaEwald, rcut, Lmin, exclude_self, use_thole, lat);
...
for b0 = 1:target_block_size:nTargets
    ...
    % Cartesian displacement from source image to target:
    %
    %   dr0 = r_target - r_source
    %
    % Row-lattice minimum image:
    %
    %   frac  = dr0 / H
    %   frac  = frac - round(frac)
    %   drMin = frac * H
    %
    % This keeps the public project convention throughout the routine.
...
if isfield(lat, 'convention')
    cache.convention = lat.convention;
else
    cache.convention = cache.lattice_convention;
end

% Backward-compatible aliases. Prefer .H/.G/.lattice in new code.
cache.lattice_rows = H;
cache.lattice_columns = H.';
...
%LOCAL_THOLE_CHARGE_CORRECTION Short-range correction to charge field.
%
% Real-space periodic Ewald charge field:
%
%   E = q * B_ewald(r) * r_vec
%
% Thole-damped short-range charge field:
%
%   E = q * f_thole(r)/r^3 * r_vec
%
% Therefore the local correction added to the Ewald real-space coefficient
% is:
%
