...
%   cutoff  : positive scalar query cutoff, must satisfy
%             cutoff <= spatial.cutoff
%
...
%   - Returned pairs are unordered and unique: i < j in the requested index
%     space.
%   - The periodic cell-list traversal may encounter the same undirected
%     pair multiple times via wrapped stencil aliases; this routine
%     canonicalizes and deduplicates before returning.
%   - Project periodic convention:
%         cart = frac * spatial.cell
%         spatial.cell rows are direct lattice vectors.
...
validateattributes(cutoff, {'double'}, ...
...
if isempty(subsetIdx)
    ...
    validateattributes(subsetIdx, {'numeric'}, ...
...
alloc = max(1024, min(spatial.n * 32, ...
    max(1024, floor(spatial.n * (spatial.n - 1) / 2))));
...
% Deduplicate canonical unordered pairs. Keep the first occurrence, stable,
% so auxiliary arrays stay aligned.
if nPairs > 0
    canon = [pairI, pairJ];
...
function [pairI, pairJ, r2All, drAll] = local_store( ...
    nPairs, pairI, pairJ, r2All, drAll, i, j, r2, dr, returnDr)
