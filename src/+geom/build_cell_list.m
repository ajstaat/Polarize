...
%       .cell                   3 x 3 lattice matrix, required when
%                               isPeriodic = true
%
%                               Project convention:
...
%                                   cart = frac * cell
%
%       .cutoff                 positive scalar search cutoff used to size
%                               bins; required for cell-list mode
%       .bin_size               optional positive scalar or 1 x 3 vector
%       .store_frac             logical, default true for periodic
%       .prefer_reach_one       logical, default true for periodic,
%                               false for nonperiodic
%       .directional_pruning    logical, default true
%       .nonperiodic_bin_scale  scalar, default 0.25
%
% Output
...
if isPeriodic
    validateattributes(cellMat, {'double'}, ...
...
% Fractional bin offset under row-vector convention.
ds = [dxyz(1) * hx, dxyz(2) * hy, dxyz(3) * hz];
ds = ds - round(ds);

% Cartesian bin-center displacement.
rc = ds * cellMat;
dc = norm(rc);
...
% Projection of row-lattice fractional-bin uncertainty onto uhat.
q = cellMat * uhat(:);
...
% Difference of two within-bin offsets ranges over:
%   [-bx,bx], [-by,by], [-bz,bz]
