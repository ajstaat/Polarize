...
% ----------------------------
% Polarize uses one public periodic convention:
%
%   H rows are direct lattice vectors
%   cart = frac * H
%   H * G = 2*pi*I
%
% k-vectors are generated by ewald.enumerate_kvecs_from_lattice, which
% accepts a sys/polsys struct, a raw row-lattice H, or a canonical
% geom.get_lattice struct.
%
...
%   .ewald                 struct with fields:
%                             .alpha
%                             .rcut
%                             .kcut
%                             .boundary, optional, 'tinfoil' or 'vacuum'
%
% Output
%   E       N x 3 field at all sites; only target rows are populated
%   parts   diagnostic struct with real/recip/surface components, masks,
%           Ewald metadata, timings, and caches
%
% Notes
% -----
% Real-space:
...
%   ewald.enumerate_kvecs_from_lattice returns one representative from each
...
%
%   In real-only mode, reciprocal k-vector enumeration and surface terms
...
validateattributes(alpha, {'numeric'}, ...
    ...
    mfilename, 'fieldParams.ewald.alpha');
validateattributes(rcut, {'numeric'}, ...
    {'scalar','real','finite','positive'}, ...
    mfilename, 'fieldParams.ewald.rcut');
validateattributes(kcut, {'numeric'}, ...
    {'scalar','real','finite','positive'}, ...
    mfilename, 'fieldParams.ewald.kcut');
...
if nSources == 0 || nTargets == 0
    if wantParts
        parts = local_make_empty_parts(sys, lat, target_mask, source_mask, ...
            target_sites, source_sites, alpha, rcut, kcut, boundary, ...
            useThole, exclude_self, kspace_mode, k_block_size, ...
            kspace_memory_limit_gb, 0.0, toc(tTotal), real_only);
    end
    return;
end
...
if real_only
    ...
    if wantParts
        parts = local_make_parts(E, Ereal_all, Erecip_all, Esurf_all, ...
            Mq, Esurf_q, surf_coeff, target_mask, source_mask, ...
            target_sites, source_sites, qtot, nTargets, nSources, ...
            realCache.nEntries, nk, alpha, rcut, kcut, boundary, ...
            useThole, exclude_self, true, storage_mode, kspace_mode, ...
            k_block_size, kspace_memory_limit_gb, estimated_full_gb, ...
            time_real, time_recip, toc(tTotal), realCache, lat, kmeta);
    end
...
if nk > 0
    ...
    %
    % enumerate_kvecs_from_lattice keeps one representative from each +/- k
...
if wantParts
    parts = local_make_parts(E, Ereal_all, Erecip_all, Esurf_all, ...
        ...
        target_sites, source_sites, qtot, nTargets, nSources, ...
        realCache.nEntries, nk, alpha, rcut, kcut, boundary, ...
        useThole, exclude_self, false, storage_mode, kspace_mode, ...
        k_block_size, kspace_memory_limit_gb, estimated_full_gb, ...
        time_real, time_recip, toc(tTotal), realCache, lat, kmeta);
end
...
function parts = local_make_parts(E, Ereal_all, Erecip_all, Esurf_all, ...
    Mq, Esurf_q, surf_coeff, target_mask, source_mask, ...
    target_sites, source_sites, qtot, nTargets, nSources, ...
    nRealEntries, nk, alpha, rcut, kcut, boundary, ...
    useThole, exclude_self, real_only, storage_mode, kspace_mode, ...
    k_block_size, kspace_memory_limit_gb, estimated_full_gb, ...
    time_real, time_recip, time_total, realCache, lat, kmeta)
...
parts.nRealEntries = nRealEntries;
parts.nK = nk;
...
function parts = local_make_empty_parts(sys, lat, target_mask, source_mask, ...
    target_sites, source_sites, alpha, rcut, kcut, boundary, ...
    useThole, exclude_self, kspace_mode, k_block_size, ...
    kspace_memory_limit_gb, time_before_return, time_total, real_only)
