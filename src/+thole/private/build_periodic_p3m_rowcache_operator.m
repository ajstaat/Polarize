function op = build_periodic_p3m_rowcache_operator(sys, problem, opt)
%BUILD_PERIODIC_P3M_ROWCACHE_OPERATOR Build SOR-oriented periodic P3M operator.
%
% Private backend builder used by thole.make_polarization_operator.
%
% Returns:
%   op.kind    = 'matrix_free'
%   op.backend = 'periodic_p3m_rowcache_apply'
%
% This backend stores:
...
% backend. solve_scf_sor should later add a periodic P3M raw fast path that
% uses op.periodic_p3m_cache directly.

io.assert_atomic_units(sys);

if isfield(opt, 'Softening') && opt.Softening ~= 0
    error('thole:build_periodic_p3m_rowcache_operator:SofteningUnsupported', ...
        'Periodic P3M row-cache operators do not support Softening ~= 0.');
end
...
op.apply_row = @(rowLocal, muVec) local_apply_periodic_p3m_row_slow( ...
    rowLocal, muVec, nPol, nSites, problem.activeSites(:), ...
    realCache, p3mCache, selfBlock, surfaceBlock);
...
op.periodic_p3m_cache.self_coeff = selfBlock(1, 1);
op.periodic_p3m_cache.surface_coeff = surfaceBlock(1, 1);
...
op.periodic_p3m_cache.use_mex_kspace = false;
op.periodic_p3m_cache.sor_fast_path_available = false;
...
op.info = struct();
op.info.assembly_backend = 'periodic_p3m_rowcache_apply';
...
function TiMu = local_apply_periodic_p3m_row_slow( ...
    rowLocal, muVec, nPol, nSites, activeSites, realCache, p3mCache, selfBlock, surfaceBlock)

if ~(isnumeric(rowLocal) && isscalar(rowLocal) && rowLocal == round(rowLocal) && ...
        rowLocal >= 1 && rowLocal <= nPol)
    error('thole:build_periodic_p3m_rowcache_operator:BadRowIndex', ...
        'rowLocal must be an integer active-site row in 1:nPolSites.');
end

Tmu = local_apply_periodic_p3m( ...
...
if numel(muVec) ~= 3*nPol
    error('thole:build_periodic_p3m_rowcache_operator:BadApplyVectorSize', ...
...
if numel(rowPtr) ~= nPol + 1
    error('thole:build_periodic_p3m_rowcache_operator:BadRowPtr', ...
...
if numel(colIdx) ~= nEntries
    error('thole:build_periodic_p3m_rowcache_operator:BadColIdxLength', ...
        'numel(rowCache.col_idx) must equal rowPtr(end)-1.');
end

if ~isempty(colIdx) && (min(colIdx) < 1 || max(colIdx) > nPol)
    error('thole:build_periodic_p3m_rowcache_operator:BadColIdx', ...
...
for k = 1:numel(required)
    ...
    if ~isfield(cache, name) || isempty(cache.(name))
        error('thole:build_periodic_p3m_rowcache_operator:MissingRealCacheField', ...
...
if size(dr, 1) ~= nEntries || size(dr, 2) ~= 3
    error('thole:build_periodic_p3m_rowcache_operator:BadDr', ...
...
if numel(coeffIso) ~= nEntries || numel(coeffDyad) ~= nEntries
    error('thole:build_periodic_p3m_rowcache_operator:BadCoeffSize', ...
...
if isempty(rowPtr)
    error('thole:build_periodic_p3m_rowcache_operator:EmptyRowPtr', ...
...
if rowPtr(1) ~= 1
    error('thole:build_periodic_p3m_rowcache_operator:BadRowPtrBase', ...
        'rowCache.row_ptr must start at 0 or 1.');
end

if any(diff(rowPtr) < 0)
    error('thole:build_periodic_p3m_rowcache_operator:BadRowPtrMonotonicity', ...
...
if isempty(p3mParams.alpha)
    error('thole:build_periodic_p3m_rowcache_operator:MissingAlpha', ...
        'Periodic P3M operator requires opt.Alpha or opt.Ewald.alpha.');
end

if isempty(p3mParams.rcut)
    error('thole:build_periodic_p3m_rowcache_operator:MissingRcut', ...
...
if ~ismember(boundary, {'tinfoil','vacuum'})
    error('thole:build_periodic_p3m_rowcache_operator:BadBoundary', ...
...
if ~isfield(opt, 'MeshSize') || isempty(opt.MeshSize)
    error('thole:build_periodic_p3m_rowcache_operator:MissingMeshSize', ...
...
if numel(meshSize) ~= 3 || any(meshSize <= 0) || any(meshSize ~= round(meshSize))
    error('thole:build_periodic_p3m_rowcache_operator:BadMeshSize', ...
...
if p3mParams.assignment_order <= 0 || p3mParams.assignment_order ~= round(p3mParams.assignment_order)
    error('thole:build_periodic_p3m_rowcache_operator:BadAssignmentOrder', ...
...
for k = 1:numel(required)
    ...
    if ~isfield(problem, name) || isempty(problem.(name))
        error('thole:build_periodic_p3m_rowcache_operator:MissingProblemField', ...
            'problem.%s is required.', name);
    end
end

if numel(problem.activeSites) ~= problem.nPolSites
    error('thole:build_periodic_p3m_rowcache_operator:BadProblemSize', ...
