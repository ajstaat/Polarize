function op = build_periodic_paircache_operator(sys, problem, opt)
%BUILD_PERIODIC_PAIRCACHE_OPERATOR Build matrix-free periodic Ewald operator.
%
% Private backend builder used by thole.make_polarization_operator.
%
% Returns:
%   op.kind    = 'matrix_free'
%   op.backend = 'periodic_paircache_apply'
%
% This backend stores:
...
% row-update capability for SOR. Use build_periodic_rowcache_operator for
% SOR-oriented operators.
%
% Required opt fields, directly or through opt.Ewald:
%   opt.Rcut or opt.Ewald.rcut
%   opt.Alpha or opt.Ewald.alpha
%   opt.Kcut or opt.Ewald.kcut
%
% Optional opt fields:
%   opt.Boundary                default 'tinfoil'
%   opt.KspaceMode              default 'auto'
%   opt.KBlockSize              default 2048
%   opt.KspaceMemoryLimitGB     default 8
%   opt.UseThole
%   opt.UseMex
%   opt.Profile
%   opt.Verbose
%   opt.Softening               must be zero for this backend

io.assert_atomic_units(sys);

if isfield(opt, 'Softening') && opt.Softening ~= 0
    error('thole:build_periodic_paircache_operator:SofteningUnsupported', ...
        'Periodic Ewald matrix-free operators do not support Softening ~= 0.');
end
...
op.info = struct();
op.info.assembly_backend = 'periodic_paircache_apply';
...
if numel(muVec) ~= 3*nPol
    error('thole:build_periodic_paircache_operator:BadApplyVectorSize', ...
...
% -------------------------------------------------------------------------
% Real-space periodic Ewald contribution
% -------------------------------------------------------------------------
...
% -------------------------------------------------------------------------
% Reciprocal-space periodic Ewald contribution
% -------------------------------------------------------------------------
...
switch storageMode
    ...
    case 'blocked'
        if ~isfield(kCache, 'blocks') || isempty(kCache.blocks)
            error('thole:build_periodic_paircache_operator:MissingKBlocks', ...
    ...
    otherwise
        error('thole:build_periodic_paircache_operator:BadKspaceStorageMode', ...
...
if numel(rowPtr) ~= nPol + 1
    error('thole:build_periodic_paircache_operator:BadRowPtr', ...
...
if numel(colIdx) ~= nEntries
    error('thole:build_periodic_paircache_operator:BadColIdxLength', ...
        'numel(rowCache.col_idx) must equal rowPtr(end)-1.');
end

if ~isempty(colIdx) && (min(colIdx) < 1 || max(colIdx) > nPol)
    error('thole:build_periodic_paircache_operator:BadColIdx', ...
...
for k = 1:numel(required)
    ...
    if ~isfield(cache, name) || isempty(cache.(name))
        error('thole:build_periodic_paircache_operator:MissingRealCacheField', ...
...
if size(dr, 1) ~= nEntries || size(dr, 2) ~= 3
    error('thole:build_periodic_paircache_operator:BadDr', ...
...
if numel(coeffIso) ~= nEntries || numel(coeffDyad) ~= nEntries
    error('thole:build_periodic_paircache_operator:BadCoeffSize', ...
...
if isempty(rowPtr)
    error('thole:build_periodic_paircache_operator:EmptyRowPtr', ...
...
if rowPtr(1) ~= 1
    error('thole:build_periodic_paircache_operator:BadRowPtrBase', ...
        'rowCache.row_ptr must start at 0 or 1.');
end

if any(diff(rowPtr) < 0)
    error('thole:build_periodic_paircache_operator:BadRowPtrMonotonicity', ...
...
if ~ismember(boundary, {'tinfoil','vacuum'})
    error('thole:build_periodic_paircache_operator:BadBoundary', ...
