...
%
% Notes
%   - When Thole damping is enabled, the damping factors are computed from
%     the bare intersite distance.
%   - Optional softening modifies the denominator powers used in the tensor
%     evaluation, but not the Thole damping factors.
    ...
    if numel(ri) ~= 3 || numel(rj) ~= 3
        error('ri and rj must each contain 3 Cartesian components.');
    end
    if ~isscalar(alpha_i) || ~isscalar(alpha_j) || ~isscalar(thole_a)
        error('alpha_i, alpha_j, and thole_a must be scalars.');
    end
