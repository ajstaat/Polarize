...
removeMolIDs = unique(removeMolIDs(:).');
...
if ~all(ismember(removeMolIDs, allMolIDs))
    ...
    error('Remove molecule ID(s) not present in system: %s', mat2str(badIDs));
end

if isfield(sys, 'active_molecules') && ~isempty(sys.active_molecules)
    overlap = intersect(removeMolIDs, sys.active_molecules);
    if ~isempty(overlap)
        error('Cannot remove molecule(s) that are also active: %s', mat2str(overlap));
    end
end

keepMask = ~ismember(sys.site_mol_id, removeMolIDs);

% Apply mask to all site-level fields if present
siteFields = { ...
    'site_pos', ...
    'site_frac', ...
    'site_mol_id', ...
...
if ~isfield(sys, 'removed_molecules') || isempty(sys.removed_molecules)
    ...
    sys.removed_molecules = unique([sys.removed_molecules(:).', removeMolIDs]);
end
