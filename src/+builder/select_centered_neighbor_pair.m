...
addParameter(p, 'PerpTol', 1e-3, ...
    @(x) isnumeric(x) && isscalar(x) && x >= 0);
addParameter(p, 'ShellTol', 1e-3, ...
    @(x) isnumeric(x) && isscalar(x) && x >= 0);

addParameter(p, 'SameStackPerpTol', 1e-3, ...
    @(x) isnumeric(x) && isscalar(x) && x >= 0);
addParameter(p, 'VectorGroupTol', 1e-3, ...
    @(x) isnumeric(x) && isscalar(x) && x >= 0);

addParameter(p, 'NormalTargetDeg', 72, ...
    @(x) isnumeric(x) && isscalar(x));
addParameter(p, 'NormalTolDeg', 15, ...
    @(x) isnumeric(x) && isscalar(x) && x >= 0);
...
% Normals are only required by the skew selector. same_stack and side_stack
% only use displacement descriptors.
includeNormals = strcmp(relation, 'skew');

descCache = builder.build_complete_molecule_descriptor_cache(sys, ...
...
for iRef = 1:numel(completeIDs)
    ...
    refCacheRow = find(descCache.molecule_id == refMolID, 1, 'first');

    if isempty(refCacheRow)
        error('builder:select_centered_neighbor_pair:BadReferenceMolID', ...
            'Reference molecule ID %d is absent from descriptor cache.', refMolID);
    end

    thisRefCOM = descCache.com(refCacheRow, :);

    [tfNbr, nbrCacheRows] = ismember(sel.match_table.molecule_id, descCache.molecule_id);

    if any(~tfNbr)
        error('builder:select_centered_neighbor_pair:BadNeighborMolID', ...
            'A selected neighbor molecule ID is absent from descriptor cache.');
    end

    thisNbrCOMs = descCache.com(nbrCacheRows, :);

    for j = 1:height(sel.match_table)
        nbrMolID = sel.match_table.molecule_id(j);

        thisNbrCOM = thisNbrCOMs(j, :);
...
result.reference_com = [
    candidateTable.ref_com_x(1), ...
    candidateTable.ref_com_y(1), ...
    candidateTable.ref_com_z(1)];
result.neighbor_com = [
    candidateTable.nbr_com_x(1), ...
    candidateTable.nbr_com_y(1), ...
    candidateTable.nbr_com_z(1)];
result.pair_midpoint = [
    candidateTable.mid_x(1), ...
    candidateTable.mid_y(1), ...
    candidateTable.mid_z(1)];
