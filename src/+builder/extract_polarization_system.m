...
%
% Inputs
%   sys     rich system struct in builder/raw units
...
%   polsys  canonical polarization system in atomic units
    ...
    if isfield(sys, 'site_label') && ~isempty(sys.site_label)
        ...
        polsys.site_label = arrayfun(@(k) sprintf('site_%d', k), 1:nSites, ...
            'UniformOutput', false).';
    end
    ...
    % ---------------------------------------------------------------------
    % Units: default builder/raw assumption is Angstrom / Angstrom^3 / e
    % ---------------------------------------------------------------------
    ...
    % ---------------------------------------------------------------------
    % Add simple source / target masks
    % ---------------------------------------------------------------------
