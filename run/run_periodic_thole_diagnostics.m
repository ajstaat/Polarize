...
% -----------------------------
% Build neutral system for lookup
% -----------------------------
opts0 = struct();
...
if isempty(pairs)
    ...
    nShow = min(10, size(pairs,1));

    fprintf('Showing %d closest polarizable pairs:\n', nShow);
    fprintf('   i    j      dist(A)        mol_i   mol_j      alpha_i      alpha_j\n');
    ...
    for p = 1:nShow
        ...
        fprintf('%4d %4d   %12.6f   %6d %6d   %10.6f   %10.6f\n', ...
...
if ~isempty(pairs)
    nShow = min(10, size(pairs,1));
    fprintf('   i    j      dist(A)      ||dTij||_F / ||Tij_real||_F\n');

    % Need Ewald params used in the actual thole run
    alphaE = res_thole.Tmeta.alpha;
    rcutE  = res_thole.Tmeta.rcut;

    % Recover H in column-vector convention
    ...
    % Find the shortest image displacement within the real-space search range used
