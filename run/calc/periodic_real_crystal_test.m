...
params_extract.ewald.mode = 'periodic';  % just for extraction path consistency
...
%% ------------------------------------------------------------------------
% Nonperiodic SCF controls

scfParams_nonper = struct();
scfParams_nonper.use_thole = cfg.use_thole;
scfParams_nonper.softening = cfg.softening;
scfParams_nonper.verbose = true;
scfParams_nonper.printEvery = 500;
scfParams_nonper.rcut = cfg.nonperiodic.rcut;
...
tProblem = tic;
problem = thole.prepare_scf_problem(polsys, Eext, scfParams_nonper);
timeProblem = toc(tProblem);
fprintf('[nonperiodic] problem prep done in %.6f s | nPolSites = %d\n', ...
    timeProblem, problem.nPolSites);

fprintf('[nonperiodic] building matrix-free pair cache...\n');
tCache = tic;
pairOpts = struct();
pairOpts.rcut = scfParams_nonper.rcut;
pairOpts.site_mask = problem.polMask;
pairCache = geom.build_nonperiodic_pair_cache(polsys, pairOpts);
timeCache = toc(tCache);
fprintf('[nonperiodic] pair cache done in %.6f s | nPairs = %d\n', ...
    timeCache, pairCache.nPairs);
