...
%   -> remove selected molecule sites from polarizable mask
...
assert(~any(sys.site_is_polarizable(chargedMask)), ...
    'Charged molecule sites should be removed from polarizable mask.');

assert(all(sys.site_alpha(chargedMask) == 0), ...
    'Charged molecule site polarizabilities should be zeroed.');
